...
#include<filesystem>
#include<ctime>
using namespace boost::filesystem;        //BOOST
namespace fs= std::experimental::filesystem;   //FILESYSTEM
...
{	
	...
	RenderWindow window = { VideoMode{windowWidth,windowHeight} , "GAME OF LIFE" };     //SFML
	...
	fs::path p{ "hardPattern.rle" };  //FILESYSTEM
	...
	shared_ptr<Cell> myBoard(new Cell[sizeX*sizeY],default_delete<Cell[]>());   //SMART POINTERS
	...
	Event event;	
		...
		for (int i = 0; i < decompressedVector3.size(); i++)
		{
			...
			cordXs[0] = pt.get<std::string>("Section1.cordX1");  //BOOST
		...
		unique_ptr<unique_ptr<Cell[]>[]>    neighboursWithTopRow;  //SMART POINTERS
